...
void compute_normals(int flip_sign);
void read_file(FILE*);
...
void normal_calc(FILE *ifp, FILE *ofp, int flip_sign)
{
  int i,j;
  char *s;
  char *progname;
  read_file(ifp);
  compute_normals(flip_sign);
  if(ofp)
      write_file(ofp);
  else
      return;
...
void write_file(FILE *fp)
{
  ...
  ply = write_ply (fp, num_elem_types, elist, in_ply->file_type);
